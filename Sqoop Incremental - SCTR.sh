...
FROM db2opr.sctr_opr, DB2MIS.IADD_MAPA_DT T3 WHERE DATA_REF = CURRENT_DATE AND DT_EST_CDU_SCTR >= T3.DATA_ANT_1 and \$CONDITIONS" \
--hbase-table 'dirco_gecig:rst_entrada' --column-family 'sctr_opr' --hbase-row-key ROW_ID \
