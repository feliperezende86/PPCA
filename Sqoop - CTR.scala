...
T1.NR_UNCO_CTR_OPR, T2.NR_EPRD_FNCD, T2.NR_SCTR_OPR, T1.SG_SIS_RSP_CDU, T1.CD_PRF_DEPE_CDU, T1.NR_CTR_OPR, T1.CD_EST_CDU, T1.DT_EST_CDU, T1.CD_CNL_FRMZ_CTR, T1.CD_PRF_DEPE_NEGR, \
T1.CD_PRF_DEPE_EXEC, T1.CD_PRD, T1.CD_MDLD, T1.CD_IEC_MOE, T1.CD_CIA_RSP_CTR \
FROM DB2OPR.CTR_OPR T1, DB2OPR.SCTR_OPR T2 \
WHERE (t1.NR_UNCO_CTR_OPR = t2.NR_UNCO_CTR_OPR) and \$CONDITIONS" \
--hbase-table 'dirco_gecig:rst506' --column-family 'ctr_opr' --hbase-row-key ROW_ID \
